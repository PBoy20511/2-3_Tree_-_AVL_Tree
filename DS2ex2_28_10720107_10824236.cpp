		    ...
		    while ( printedIndex.size() < dataSize() ) {
                ...
                for( int j = 0 ; j < data[minInputnum].sExtra.size() ; j++ ) {
                        ...
                        cout << num << ". "
						     ...
						     << data[minInputnum].sExtra[j].numStudent << endl ;
...
class TTTree {
	...
	public:
		...
		void PrintDepartment( string dName, vector<CollegeType> sExtra ){
      // cout << "in" ;
      if( dName == "*" ){
        for( int i=0; i<sExtra.size(); i++ ){
          sExtra[i].print() ;
        } // for
      } // if
      else{
        for( int i=0; i<sExtra.size(); i++ ){
          if( strcmp(sExtra[i].nameMajor.c_str(), dName.c_str())==0 ){
            sExtra[i].print() ;
          } // if
        } // for
      } // else
    ...
    bool Search( string sName, string dName , Node23 * rot ) { // inserts in into the 2-3 tree through rot traversal

      if( rot == NULL ) { // first insertion
        return false;
      } // end if
      else if( rot->twoNode() ) { // is a 2-node
        // cout << rot->name1().c_str() << " " <<sName.c_str() << "twoNode()\n";
        if( strcmp( sName.c_str() , rot->name1().c_str() ) == 0 ) {
          // cout << "3\n";
          PrintDepartment( dName, rot->data[0].sExtra ) ;
          return true;
        } // if
        else if( strcmp( sName.c_str() , rot->name1().c_str() ) > 0 ) {
          Search( sName, dName, rot->child[2]) ;
        } // else if
        else {
          Search( sName, dName, rot->child[0] ) ;
        }
      } // end else if
      else { // is a 3-node
        // cout << rot->name1() << " " <<sName.c_str() << "threeNode()\n";
        if( strcmp( sName.c_str() , rot->name1().c_str() ) <= 0 ) {
          if( strcmp( sName.c_str() , rot->name1().c_str() ) == 0 ){
            // cout << "4\n";
            PrintDepartment( dName, rot->data[0].sExtra ) ;
            ...
            Search( sName, dName, rot->child[0] ) ; // walks left
          }
        }
        else if( strcmp( sName.c_str() , rot->name2().c_str() ) >= 0 ) {
          if( strcmp( sName.c_str() , rot->name2().c_str() ) == 0 ){
            // cout << "5\n";
            PrintDepartment( dName, rot->data[1].sExtra ) ;
            return true;
          }
          else {
            Search( sName, dName, rot->child[2]) ; // walks right
          }
        }
        else {
          Search( sName, dName, rot->child[1]) ; // walks mid
        }
...
class AVLTree {
	...
	public:
		...
		AVLTreeNode* Insert( AVLTreeNode* walk, CollegeType department ) { 
			...
			else if ( strcmp( department.nameMajor.c_str(), walk->list[0].nameMajor.c_str() ) == 0 ) {
				walk->list.push_back( department ) ;
				return walk ;
			} // else if
			else if ( strcmp( department.nameMajor.c_str(), walk->list[0].nameMajor.c_str() ) < 0 ) {
		...
		void TopKGraduates( AVLTreeNode* walk, int& k, int& printed ) {
			if ( walk == NULL ) {
				;
			}
			else if ( k > 0 ) {
				TopKGraduates( walk->right, k, printed ) ;
				if ( k > 0 ) {
					for ( int i = 0 ; i < walk->list.size() ; i++ ) {
						printed++ ;
						cout << printed << ". "
				...
				TopKGraduates( walk->left, k, printed ) ;
			}
		} // TopKGraduates()
		...
		AVLTreeNode* search( AVLTreeNode* walk, string department ) {
			
			if ( walk == NULL )
				return walk;
				
			else if ( strcmp( department.c_str(), walk->list[0].nameMajor.c_str() ) == 0 ) {
				return walk;
			} // else if
			
			else if ( strcmp( department.c_str(), walk->list[0].nameMajor.c_str() ) > 0 ) {

				return search( walk->right, department );
			} // else if
			
			else if ( strcmp( department.c_str(), walk->list[0].nameMajor.c_str() ) < 0 ) {

				return search( walk->left, department );
			} // else if
...
int main() {
	cout << "(1) 2-3 tree" << endl 
	     << "(2) AVL tree" << endl
	     << "(3) Intersection Query" << endl
			 << "(4) compare exe speed" << endl
	     << "(0) Exit" << endl
	     << "Mission: " ;
	int cmd = 0 ;
	cin >> cmd ;
	...
	while ( cmd != 0 ) {
		if ( cmd != 1 && cmd != 2 && cmd != 3 && cmd!=4 ) {
			...
			if ( !listT.Load(fileNum) ) {
				...
				avlTree.Initialize() ;
				avlTree.Build( listT.collegeSet ) ;
				...
				if ( strcmp( school.c_str(), "*" ) == 0 && strcmp( major.c_str(), "*" ) == 0 ) {
					listT.Print();
				} // if
				
				else if ( strcmp( school.c_str(), "*" ) == 0 ) {
					AVLTreeNode* node = avlTree.search( avlTree.root, major );

					if ( node == NULL )
						cout << "doesn't exist." << endl;
					else
						avlTree.PrintNode( node );
				} // else if
				
				else {
					if ( tttree.Search( school, major, tttree.root ) ) {
						;
					} // if
      ...
      string fileNum ;
        ...
        tttree.reset() ;
				...
				int c = 0;
				tttree.numOfNodes(tttree.root, c) ;
				cout << "2-3tree has " << c << " nodes.\n";
				cout << "avltree has " << avlTree.NodeTotal(avlTree.root) << " nodes.\n" ;
			} // else
